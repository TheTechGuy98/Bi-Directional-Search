...
def check(openf, closedf, state, gfc):
    ...
    return openf_index, closedf_index, check, gfc_check
...
def reverse_actions(final_actions_openb):
    curr_actions=final_actions_openb[:-1]
    reversed_action=[]
    ...
    while not (openf.isEmpty() and openb.isEmpty()):
        ...
        if prmins_values.index(min(prmins_values)) == 0:  # which we are in openf condition
            ...
            for state in states:
                ...
                openf_index, closedf_index, node_check, gfc_check = check(openf, closedf, state, gfc)
                if node_check and gfc_check:
                    continue
                if node_check:
            ...
            for state in states:
                ...
                openb_index, closedb_index, node_check, gfc_check = check(openb, closedb, state, gfc)
                if node_check and gfc_check:
                    continue
                if node_check:
